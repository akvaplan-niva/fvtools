...
def extract2file_subset_node_var(
    ...
    for file_name, index_range in file_indices.items():
        ...
        if first_time:
            if subset is None:
                total_cells = d['zeta'].shape[1]  # xarray DataArray, same as before
        ...
        indexers = {'time': slice(start_idx, end_idx),'node': subset.tolist()}
        for var in variables:    
