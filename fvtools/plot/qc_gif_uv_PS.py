...
def main(filelist = None,
    ...
    UV           = UVmov(M, fl, stream, xlim, ylim, hres, avg, vlev, clim=clim, dpi = dpi)
    UV.sig, UV.z = sigma, z
...
class UVmov:
    ...
    '''
    def __init__(self, M, fl, stream, xlim, ylim, hres, avg, vlev = None, clim = None, verbose = True, dpi = 300):
        '''
        ...
        # Prepare figure with South Polar Stereographic projection
        self.fig = plt.figure(figsize=(8, 8), dpi=dpi)
        self.ax  = self.fig.add_subplot(111, projection=ccrs.SouthPolarStereo(central_longitude=0))
        self.ax.add_feature(cfeature.COASTLINE, linewidth=0.4, zorder=10)
        self.ax.gridlines(draw_labels=False, linewidth=0.3, alpha=0.4)

        if str(self.M.reference).endswith('3031'):
            pad = 1e5
            self.ax.set_xlim(float(self.M.x.min()) - pad, float(self.M.x.max()) + pad)
            self.ax.set_ylim(float(self.M.y.min()) - pad, float(self.M.y.max()) + pad)
            self.M.transform = None
            self._use_geo = False
        else:
            lonmin, lonmax = float(self.M.lon.min()), float(self.M.lon.max())
            latmin = max(-90.0, float(self.M.lat.min()))
            latmax = float(self.M.lat.max())
            self.ax.set_extent([lonmin, lonmax, -90.0, max(latmax, -50.0)], crs=ccrs.PlateCarree())
            self.M.transform = ccrs.PlateCarree()
            self._use_geo = True
    ...
    def animate(self, i):
        ...
        try:
            c = self.M.plot_contour(self.sp_fv, levels = self.vlev, cmap = 'jet', extend = 'both',
                                    vmin=self.clim[0] if hasattr(self, "clim") and self.clim else None,
                                    vmax=self.clim[1] if hasattr(self, "clim") and self.clim else None,
                                    show = False)
