...
from fvtools.grid import Grid
from fvtools.util import num2date


def main(start, stop, vassdrag, gridfile, info=None, temp=None):
    """
    ...
    temp:      Path to the RiverTemperature.npy file you want to use
    ...
    Temp    = RiverTemperatures(info, vassdrag, M.info['casename'])
...
class RiverTemperatures():
    ...
    """
    def __init__(self, info, vassdrag, casename):
        """
    ...
    def compile_temperature(self):
        ...
        for _file in self.files:
            ...
            plt.figure()
            plt.plot(_data['datetime'], _data['temp'])
            plt.title(_file)
        ...
        # Convolve to reduce the most significant noise?
        # Get a "typical" temperature on a given day, use it to fill missing days other years
        # ----
        for _data in data:
        ...
        # Loop to interpolate river temperatures to rivertime
        # ----
        ...
        for i, _data in enumerate(data):
            ...
            _data['year_temp'] = np.append(_data['year_temp'], _data['year_temp'][-1]) # Not correct, leap years should be extended in february, not december
        ...
        for i, temp in enumerate(self.river_temp.T):
            ...
            plt.title(self.files[i].split('.csv')[0])
            plt.show(block = False)
        ...
        # Make sure to remove negative fresh water
        # ----
        ...
        np.save(self.info['rivertemp']+'/'+self.casename+'_temperatures.npy', data)
...
class FVCOM_rivers():
    ...
    def __init__(self, info, M, vassdrag):
        ...
        self.land_tree   = KDTree(np.array([self.x_land[:,0], self.y_land[:,0]]).transpose())
        self.obc_tree    = KDTree(np.array([self.x_obc[:,0],  self.y_obc[:,0]]).transpose())
        if self.info['iloc'] == 'edge':
            self.mesh_tree   = KDTree(np.array([self.M.xc[:,0], self.M.yc[:,0]]).transpose())
        elif self.info['iloc'] == 'node':
            self.mesh_tree   = KDTree(np.array([self.M.x[:,0],  self.M.y[:,0]]).transpose())
    ...
    def make_time(self, start, stop, Runoff, Temp, dt = 3/24):
        ...
        # These fields will be dumped to
    ...
    def crop_river_to_obc(self, river_object):
        ...
        d, obc_ind   = self.obc_tree.query(np.array([self.x_land[land_ind[0,close_enough],0], \
                                                     self.y_land[land_ind[0,close_enough],0]]).transpose())
    ...
    def connect_to_mesh(self):
        ...
        while True:
            ...
            self.mesh_location = mesh_location[0]
    ...
    def merge_rivers(self):
        ...
        if self.info['iloc'] == 'edge':
            self.xriv    = self.M.xc[self.unique_mesh,0]
            self.yriv    = self.M.yc[self.unique_mesh,0]
    ...
    def river_stability(self):
        ...
        if self.info['iloc'] == 'edge':
            ...
            cell = KDTree(np.array([self.M.xc[:,0], self.M.yc[:,0]]).transpose())
        ...
        if self.info['iloc'] == 'edge':
            ...
            h  = hc[self.unique_mesh,0]-self.info['tideamp']

        elif self.info['iloc'] == 'node':
            h  = self.M.h[self.unique_mesh, 0]-self.info['tideamp']
    ...
    def split_problematic_river(self, bad_rivers):
        ...
        for river in bad_rivers:
            ...
            d, this_land  = self.land_tree.query(np.array([self.xriv[river], self.yriv[river]]).transpose(),
                                                 k = n_newland)
            # ----
            new_x         = np.copy(self.x_land[this_land])[:,0]
            new_y         = np.copy(self.y_land[this_land])[:,0]
    ...
    def river_connection(self, land_loc):
        ...
        xvec = np.array([xl[:,0], self.xriv]).transpose()
        yvec = np.array([yl[:,0], self.yriv]).transpose()
...
def show_forcing(obj):
    ...
    plt.scatter(obj.xriv, obj.yriv, obj.RiverTransport.max(axis = 0), \
                c = obj.RiverTransport.max(axis = 0))
    plt.title('Transport')
    plt.axis('equal')
