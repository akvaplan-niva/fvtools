...
sys.path.append('/work/hdj002/python_script/fvcom_pytools')
...
def main(savename     = None, 
         ...
         Sc_scale     = 1.0):
    """
    Write flux from different locations to "savename".nc file.
    ------
    ...
    if not hasattr(M, 'art1'):
        raise KeyError(grd_file+' does not contain art1. Normally works with a casename_0001.nc type of output.')
...
def write_netcdf_flux(savename, start, stop, M, carbon, cages, flux_period_seconds, gram):
    ...
    outfile.createDimension('node', M.x.shape[0])
