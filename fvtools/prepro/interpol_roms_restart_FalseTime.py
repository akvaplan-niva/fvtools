...
# changing the in def main(....., proj='epsg:32633'.....) to def main(....., proj='epsg:32633'.....)
def main(restartfile, mother, uv=False, proj='epsg:3031', latlon = False, time_ERROR = 0):
    '''
    ...
    time_ERROR    - set the error margin for difference in time between roms and fvcom files
    '''
    # FVCOM grid object
    # ----
    ...
    Restarter = Roms2FVCOMRestart(restartfile, N4, M.tri, variables, latlon, verbose=False, time_ERROR=time_ERROR)
    ...
    Restarter = Roms2FVCOMRestart(restartfile, N4, M.tri, variables, latlon, verbose=True, time_ERROR=time_ERROR) #re-initializing the interpolator just to make sure that everything is ready...
    timestep = Restarter.download(variables = variables)
...
class Roms2FVCOMRestart(RomsDownloader, LinearInterpolation):
    ...
    '''
    def __init__(self, restartfile, N4, tri, variables, latlon, time_ERROR = 0, verbose = False):
        '''
    ...
    def find_roms_to_start_from(self, verbose):
        ...
        with netCDF4.Dataset(self.restartfile) as nc:
            # prepare search window start_time to stop_time
            self._prepare_search()
            # get all the times from ROMS files
            time, self.path, self.indices = rn.make_fileList(self.start_time, self.stop_time, self.ROMS)
            # get the roms time that matches exactly with fvcom time
            #ind = np.where(time==nc['time'][0])

            # get fvcom time
            fvcom_TIME = nc['time'][0]
            # find the roms index that is closest to fvcom time
            closest_roms_index = np.argmin(np.abs(time - fvcom_TIME))

        time_difference = np.abs(time[closest_roms_index] - fvcom_TIME)
        ...
        if time_difference > self.time_error: 
                 raise CouldNotFindRestartTime(f"Found nearest time, but time difference {time_difference} exceeds threshold {self.time_error}.")

        self.path  = self.path[closest_roms_index]
        self.index_here = self.indices[closest_roms_index]
